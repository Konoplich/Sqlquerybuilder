...
class DatabaseTest
{
    ...
    public function testBuildQuery(): void
    {
        ...
        $results[] = $this->db->buildQuery('SELECT name FROM users WHERE user_id = ?f',[0.001]);
        $results[] = $this->db->buildQuery('SELECT name FROM users WHERE user_id = ?',[0.001]);
        ...
        $correct = [
            ...
            'SELECT name FROM users WHERE user_id = 0.001',
            'SELECT name FROM users WHERE user_id = 0.001',
